...
#include "block1_nebus/srv/move_to_pose.hpp"
...
{
public:
  MotionManagerNode() : Node("motion_manager_6dof_node")
  {
    ...
    best_ik_client_ =
      this->create_client<block1_nebus::srv::GetBestIkSolution6Dof>(
    ...
    // vstup = cielova poloha a orientacia toolu
    // manager sam vybavi zvysok
    move_to_pose_srv_ =
      this->create_service<block1_nebus::srv::MoveToPose>(
        "move_to_pose",
        std::bind(
          &MotionManagerNode::handle_move_to_pose,
          this,
          std::placeholders::_1,
          std::placeholders::_2
        ),
    ...
    RCLCPP_INFO(this->get_logger(), "motion_manager_6dof_node started.");
  }
...
private:
  ...
  // =========================================================
  //
  // Ukladame si aktualne uhly joint_1 az joint_6.
  // Potrebujeme ich:
  ...
  std::vector<double> compute_synchronized_velocities(
    double target_q1, double target_q2, double target_q3,
    double target_q4, double target_q5, double target_q6) const
  {
    ...
    const double dq4 = std::abs(normalize_angle(target_q4 - current_q4_));
    const double dq5 = std::abs(normalize_angle(target_q5 - current_q5_));
    const double dq6 = std::abs(normalize_angle(target_q6 - current_q6_));

    const double max_delta = std::max({dq1, dq2, dq3, dq4, dq5, dq6});
    const double min_velocity = 0.05;  // mala nenulova rychlost, aby joint "nestal"

    std::vector<double> velocities(6, min_velocity);
    ...
    velocities[3] = std::max(min_velocity, max_velocity_ * (dq4 / max_delta));
    velocities[4] = std::max(min_velocity, max_velocity_ * (dq5 / max_delta));
    velocities[5] = std::max(min_velocity, max_velocity_ * (dq6 / max_delta));

    return velocities;
  }

  // =========================================================
  // 3. SERVICE HANDLER PRE move_to_pose
  // =========================================================
  //
  // Toto je hlavna logika celeho motion managera.
  // Ked pouzivatel zavola service move_to_pose:
  ...
  void handle_move_to_pose(
    const std::shared_ptr<block1_nebus::srv::MoveToPose::Request> request,
    std::shared_ptr<block1_nebus::srv::MoveToPose::Response> response)
  {
    ...
    if (!best_ik_client_->wait_for_service(std::chrono::seconds(3))) {
      response->success = false;
      response->message = "Service get_best_ik_solution_6dof is not available.";
      return;
    }

    // Zlozime request na IK solver
    auto ik_request =
      std::make_shared<block1_nebus::srv::GetBestIkSolution6Dof::Request>();
    ik_request->x = request->x;
    ik_request->y = request->y;
    ik_request->z = request->z;
    ik_request->qx = request->qx;
    ik_request->qy = request->qy;
    ik_request->qz = request->qz;
    ik_request->qw = request->qw;
    ...
    const double target_q4 = normalize_angle(ik_result->q4);
    const double target_q5 = normalize_angle(ik_result->q5);
    const double target_q6 = normalize_angle(ik_result->q6);

    // Pripravime rychlosti tak, aby vsetky jointy prisli priblizne naraz
    std::vector<double> velocities =
      compute_synchronized_velocities(
        target_q1, target_q2, target_q3,
        target_q4, target_q5, target_q6
      );
  ...
  rclcpp::Client<block1_nebus::srv::GetBestIkSolution6Dof>::SharedPtr best_ik_client_;
  rclcpp::Client<rrm_msgs::srv::Command>::SharedPtr move_command_client_;

  // Service pre pouzivatela
  rclcpp::Service<block1_nebus::srv::MoveToPose>::SharedPtr move_to_pose_srv_;
